...
public:
    ...
    //
    virtual bool make_entrustID(char* buffer, int length){}
    //登录
    virtual int login(){}
    //登出
    virtual int logout(){}
    //下达委托
    virtual int order_insert(Entrust* entrust){}
    //
    virtual int order_action(EntrustAction* action){}
    //查询账户
    virtual int query_account(){}
    //查询持仓
    virtual int query_positions(){}
    //查询历史订单
    virtual int query_orders(){}
    //查询历史成交
    virtual int query_trades(){}
    //查询指定日期的结算单
    virtual int query_settlement(uint32_t uDate){}
};
