...
public:
    TradingDayTplMap m_tradingDayTplMap;    //{"CHINA" : TradingDayTpl}
    SessionCodeMap m_sessionCodeMap;        //{"FD0900" : CodeSet}, 记录属于某交易时间段类型的所有品种
    ExchgContract* m_exchgContractMap;      //{"SHFE" : ContractList*}, 记录属于某交易所的所有合约以及合约信息
    SessionMap* m_sessionMap;               //{"FD0900" : SessionInfo*}, 记录某交易时间段类型的交易时间段信息
    CommodityMap* m_commodityMap;           //{SHFE.rb : CommodityInfo*}, 记录某品种的品种信息
    ContractMap* m_contractMap;             //{"rb2312" : [ContractInfo*]}, 记录某合约的合约信息(为什么以Array的形式)
