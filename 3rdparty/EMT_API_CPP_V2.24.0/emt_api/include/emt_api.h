...
/*******************************************************************回调函数定义*******************************************************************/
    ...
    ///@param session_id 资金账户对应的session_id，登录时得到
    ///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
    typedef void (*EMT_OnQueryOfferTgwId)(EMTQueryOfferTgwIdRsp *offer_tgw_info, EMTRI *error_info, int request_id, bool is_last, uint64_t session_id);
    ...
    ///@param session_id 资金账户对应的session_id，登录时得到
    ///@remark 当rsp_sequence为0，表明当次查询没有查到任何记录，当is_last为true时，如果rsp_sequence等于req_count，那么表示还有回报，可以进行下一次分页查询，如果不等，表示所有回报已经查询完毕。一个查询请求可能对应多个响应，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线。
    typedef void (*EMT_OnQueryDMATickerAssignInfoByPage)(EMTQueryDMATickerAssignInfo *assign_info, int64_t req_count, int64_t rsp_sequence, int64_t query_reference, int request_id, bool is_last, uint64_t session_id);
    /*******************************************************************请求函数定义*******************************************************************/
    ...
    EMTAPI_EXPORTS int  RegistCallBack(
        ...
        EMT_OnCreditCashRepay                 cb20, EMT_OnCreditCashRepaySpecifiedDebt    cb21, EMT_OnQueryCreditCashRepayInfo        cb22, EMT_OnQueryCreditFundInfo               cb23,
        ...
        EMT_OnQueryOptionCombinedOrders       cb36, EMT_OnQueryOptionCombinedOrdersByPage cb37, EMT_OnQueryOptionCombinedTrades         cb38,
        EMT_OnQueryOptionCombinedTradesByPage cb39, EMT_OnQueryOptionCombinedPosition     cb40, EMT_OnQueryOptionCombinedStrategyInfo cb41, EMT_OnQueryOptionCombinedExecPosition   cb42,
        EMT_OnQueryOtherServerFund            cb43, EMT_OnQueryCreditMarginRate           cb44, EMT_OnQueryCreditPositionFullRate     cb45, EMT_OnQueryCreditDebtInfoByPage         cb46,
        EMT_OnQueryCreditTickerAssignInfoByPage cb47, EMT_OnQueryCreditPledgeStkByPage cb48, EMT_OnQueryCreditTargetStkByPage cb49, EMT_OnQueryCreditPledgeStkRate          cb50,
        EMT_OnQueryIssueInfoList              cb51, EMT_OnQuerySecurityInfo               cb52, EMT_OnCreditQuotaTransfer             cb53, EMT_OnQueryCreditQuotaTransfer          cb54,
        EMT_OnQueryYesterdayAsset             cb55, EMT_OnQueryETFByPage                  cb56, EMT_OnQuerySecurityByPage             cb57, EMT_OnOptionExerciseOrderEvent          cb58,
        EMT_OnQueryHKConnectIndicativeExchangeRateInfo cb59, EMT_OnQueryHkConnectSpreadTable cb60, EMT_OnHoldingChangeEvent           cb61, EMT_OnQueryCreditDebtToRepayTodayByPage cb62,
        EMT_OnQueryFund                       cb63, EMT_OnQueryOfferTgwId           cb64, EMT_OnQueryDMATickerAssignInfoByPage cb65);
    ...
    ///查询可指定报盘网关号
    ///@return 查询是否成功，“0”表示成功，非“0”表示出错，此时用户可以调用GetApiLastError()来获取错误代码
    ///@param query_param 查询条件,需要指定交易所编号
    ///@param session_id 资金账户对应的session_id,登录时得到
    ///@param request_id 用于用户定位查询响应的ID，由用户自定义
    EMTAPI_EXPORTS int QueryOfferTgwId(const EMTQueryOfferTgwIdReq *query_param, uint64_t session_id, int request_id);

    ///分页请求查询查询DMA业务融入股份信息
    ///@return 查询是否成功，“0”表示成功，非“0”表示出错，此时用户可以调用GetApiLastError()来获取错误代码
    ///@param query_param 需要分页查询查询DMA业务融入股份信息的条件，如果第一次查询，那么query_param.reference填0
    ///@param session_id 资金账户对应的session_id，登录时得到
    ///@param request_id 用于用户定位查询响应的ID，由用户自定义
    ///@remark 该方法支持分页查询，注意用户需要记录下最后一笔查询结果的reference以便用户下次查询
    EMTAPI_EXPORTS int QueryDMATickerAssignInfoByPage(const EMTQueryDMATickerAssignByPageReq *query_param, uint64_t session_id, int request_id);
#ifdef __cplusplus
}
#endif
