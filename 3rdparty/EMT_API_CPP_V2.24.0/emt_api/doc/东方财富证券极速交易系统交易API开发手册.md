<font size="6">**东方财富证券极速交易系统交易API开发手册(Ver2.24.0)**</font><br>
...
<table><tbody>
    ...
    <tr>
        <td>20250107</td><td>2.23.1</td>
        <td>
            Level-2行情API支持内核socket接收行情 <br>
        </td>
    </tr>
    <tr>
        <td>20250607</td><td>2.24.0</td>
        <td>
            新增现货可指定报盘网关号查询接口:QueryOfferTgwId <br>
            现货委托接口支持指定报盘网关号 <br>
            新增DMA融券股份信息查询接口:QueryDMATickerAssignInfoByPage <br>
            持仓查询响应(EMTQueryStkPositionRsp)增加信用持仓余额creditstkbal、信用持仓可用creditstkavl <br>
            两融指定偿还负债接口(CreditCashRepaySpecifiedDebt)支持指定偿还账户级费用，新增入参偿还负债类型debt_type <br>
            成交推送、成交查询响应结构体(EMTTradeReport)删除已废弃字段local_order_id <br>
            部分错误码描述更新 <br>
        </td>
    </tr>
</table>
...
### 3.2.63. <a id="queryoffertgwid">请求查询可指定报盘网关号</a>

| 接口名称 | QueryOfferTgwId      |                                                       |                                              |
| :------- | :----------------------- | :---------------------------------------------------- | :------------------------------------------- |
| 功能描述 | 请求查询可指定报盘网关号 |                                                       |                                              |
|          | <b>名称</b>              | <b>类型及长度</b>                                     | <b>描述</b>                                  |
| 入参     | 需要查询的报盘网关号条件 | [EMTQueryOfferTgwIdReq](#emtqueryoffertgwidreq) * | 类型参见入参结构体定义                       |
|          | 会话号                   | uint64_t  session_id                                  | 资金账户对应的session_id,登录时得到          |
|          | 请求序号                 | int       request_id                                  | 用于用户定位查询响应的ID，由用户自定义       |
| 出参     | 函数返回值               | int                                                   |                                              |
|          | 可指定报盘网关号查询响应 | [EMTQueryOfferTgwIdRsp](#emtqueryoffertgwidrsp)*  | 类型参见出参结构体定义；异步调用回调函数返回 |
|          | 错误响应结构体           | [EMTRspInfoStruct](#emtrspinfostruct)*                | 类型参见出参结构体定义                       |
|          | 请求序号                 | int       request_id                                  | 此消息响应函数对应的请求ID                   |
|          | 会话号                   | uint64_t  session_id                                  | 资金账户对应的session_id,登录时得到          |
| 备注信息 |                          |                                                       |                                              |

### 3.2.64. <a id="querydmatickerassigninfobypage">分页请求查询查询DMA业务融入股份信息</a>

| 接口名称 | QueryDMATickerAssignInfoByPage      |                                                       |                                              |
| :------- | :----------------------- | :---------------------------------------------------- | :------------------------------------------- |
| 功能描述 | 分页请求查询查询DMA业务融入股份信息 |                                                       |                                              |
|          | <b>名称</b>              | <b>类型及长度</b>                                     | <b>描述</b>                                  |
| 入参     | 需要分页查询查询DMA业务融入股份信息的条件 | [EMTQueryDMATickerAssignByPageReq](#emtquerydmatickerassignbypagereq) * | 类型参见入参结构体定义                       |
|          | 会话号                   | uint64_t  session_id                                  | 资金账户对应的session_id,登录时得到          |
|          | 请求序号                 | int       request_id                                  | 用于用户定位查询响应的ID，由用户自定义       |
| 出参     | 函数返回值               | int                                                   |                                              |
|          | DMA业务融入股份信息       | [EMTQueryDMATickerAssignInfo](#emtquerydmatickerassigninfo)*  | 类型参见出参结构体定义；异步调用回调函数返回 |
|         |请求到的最大数量             |int64_t   req_count                   ||
|         |分页请求的当前回报数量       |int64_t   rsp_sequence               ||
|         |当前报单信息所对应的查询索引  |int64_t   query_reference              |需要记录下来，在进行下一次分页查询的时候需要用到|
|         |请求序号                    |int       request_id                   |此消息响应函数对应的请求ID|
|         |结束标识                    |bool      is_last                      |当为最后一个的时候为true，如果为false，表示还有其他后续消息响应|
|         |会话号                      |uint64_t  session_id                   |资金账户对应的session_id,登录时得到|
...
|报盘网关号ID          |offer_tgw_id          |uint8_t        |N|仅具备指定报盘网关号权限的客户填写|
|预留空间 |reserved | char              |N||
|备注信息          |                 |                 |             ||
...
struct EMTCrdRepaySpecifiedDebtReq
{
    ...
    char                debt_id[16];
    ///还负债类型
    EMT_REPAY_SPECIAL_DEPT_TYPE debt_type;
    ///预留字段
    char                reserved[31];
};
```
...
| 还负债类型    | debt_type         | enum         | N         | 参见字典定义<br>[EMT_REPAY_SPECIAL_DEPT_TYPE](#emt_repay_special_dept_type) |
| 预留            | reserve         | char[31]     | N         |                                                                       |
| 备注信息        |                 |              |           |                                                                       |
...
### 3.3.30. <a id="emtqueryoffertgwidreq">EMTQueryOfferTgwIdReq</a>

```cpp
//////////////////////////////////////////////////////////////////////////
///客户可指定报盘网关号查询请求结构体
//////////////////////////////////////////////////////////////////////////
struct EMTQueryOfferTgwIdReq{
    EMT_EXCHANGE_TYPE    exchange_id;             ///< 交易所代码
    char                 reserve[12];             ///< 预留字段
};
```

| 客户可指定报盘网关号查询请求结构体 | EMTQueryOfferTgwIdReq |                   |             |                                                         |
| :--------------------------------- | :---------------------- | :---------------- | :---------- | :------------------------------------------------------ |
| <b>变量名称</b>                    | <b>标识</b>             | <b>类型及长度</b> | <b>必要</b> | <b>描述</b>                                             |
| EMT_EXCHANGE_TYPE                  | exchange_id             | enum              | Y           | 参考字典定义<br>[EMT_EXCHANGE_TYPE](#emt_exchange_type) |
| 预留字段                           | reserve                 | char[12]          | N           |                                                         |
| 备注信息                           |                         |                   |             |                                                         |

### 3.3.31. <a id="emtquerydmatickerassignbypagereq">EMTQueryDMATickerAssignByPageReq</a>

```cpp
//////////////////////////////////////////////////////////////////////////
///查询DMA业务融入股份信息请求结构体
//////////////////////////////////////////////////////////////////////////
typedef EMTQueryByPageReq EMTQueryDMATickerAssignByPageReq;
```
...
|算法类型          |algorithm_type          |uint8_t            |N            |无需用户填写|
|预留空间          |reserved                |char            |N||
|报盘网关号ID |offer_tgw_id |uint8_t |N|该笔订单申报的报盘网关号|
...
| 合约代码            | ticker          | char[16]     | Y         |客户端请求不带空格，以'\0'结尾|
| 交易市场            | market          | enum         | Y         |参见字典定义<br>[EMT_MARKET_TYPE](#emt_market_type)|
| 报单价格条件          | price_type      | enum         | Y         |参见字典定义<br>[EMT_PRICE_TYPE](#emt_price_type)|
| 预留空间            | reserved        | char[4]      | N         ||
...
struct EMTQueryStkPositionRsp
{
    ...
    /// 信用持仓余额
    int64_t             creditstkbal;
    /// 信用持仓可用
    int64_t             creditstkavl;

    ///(保留字段)
    uint64_t unknown[50 - 8];
};
```
...
|信用持仓余额            |creditstkbal            |int64_t        |保留字段，目前为0|
|信用持仓可用            |creditstkavl            |int64_t        |保留字段，目前为0|
|保留字段                |unknown                 |uint64_t[42]   |保留字段，目前为0|
...
| 还负债类型               | debt_type                 | enum      | 参见字典定义<br>[EMT_REPAY_SPECIAL_DEPT_TYPE](#emt_repay_special_dept_type)         |
...
### 3.4.52. <a id="emtqueryoffertgwidrsp">EMTQueryOfferTgwIdRsp</a>

```cpp
//////////////////////////////////////////////////////////////////////////
///客户可指定报盘网关号查询应答结构体
//////////////////////////////////////////////////////////////////////////
struct EMTQueryOfferTgwIdRsp{
    EMT_EXCHANGE_TYPE             exchange_id;          ///< 交易所代码
    uint8_t                       offer_tgw_id;         ///< 报盘网关号
    char                          reserve[24];          ///< 预留字段
};
```

| 客户可指定报盘网关号查询响应结构体 | EMTQueryOfferTgwIdRsp |                   |                                                              |
| :--------------------------------- | :---------------------- | :---------------- | :----------------------------------------------------------- |
| <b>变量名称</b>                    | <b>标识</b>             | <b>类型及长度</b> | <b>描述</b>                                                  |
| 交易所代码                         | exchange_id             | enum              | 参考字典定义<br>[EMT_EXCHANGE_TYPE](#emt_exchange_type)      |
| 报盘网关号                         | offer_tgw_id        | uint8_t           |                                                              |
| 预留字段                           | reserve                 | char[24]          |                                                              |

### 3.4.53. <a id="emtquerydmatickerassigninfo">EMTQueryDMATickerAssignInfo</a>

```cpp
//////////////////////////////////////////////////////////////////////////
///查询DMA业务融入股份信息的响应结构体
//////////////////////////////////////////////////////////////////////////
struct EMTQueryDMATickerAssignInfo {
    EMT_MARKET_TYPE market;                     ///< 证券市场
    char ticker[EMT_TICKER_LEN];                ///< 证券代码
    int64_t loan_bal;                           ///< 融入股份余额
    int64_t loan_avl;                           ///< 融入股份可用
    int64_t repay_qty;                          ///< 待还股份数量
    double repay_cost_price;                    ///< 待还股份成本价
};
```

| 查询DMA业务融入股份信息的响应结构体           | EMTQueryDMATickerAssignInfo |              |                                                              |
|:------------------------------|:----------------------------|:-------------| :----------------------------------------------------------- |
| <b>变量名称</b>                   | <b>标识</b>                   | <b>类型及长度</b> | <b>描述</b>                                                  |
| 证券市场                          | market                      | enum         | 参考字典定义<br>[EMT_MARKET_TYPE](#emt_market_type)      |
| 证券代码                          | ticker                      | char[16]     |                                                              |
| 融入股份余额                        | loan_bal                    | int64_t      | |
| 融入股份可用                        | loan_avl                    | int64_t      |                                                              |
| 待还股份数量                        | repay_qty                   | int64_t      |                                                              |
| 待还股份成本价                       | repay_cost_price            | double       |                                                              |
...
### 4.1.66. <a id="onqueryoffertgwid">OnQueryOfferTgwId</a>

```cpp
///查询可指定报盘网关号响应，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
///@param offer_tgw_info 可指定报盘网关号信息
///@param error_info 查询昨日资产发生错误时返回的错误信息，当error_info为空，或者error_info.error_id为0时，表明没有错误
///@param request_id 此消息响应函数对应的请求ID
///@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示有其他后续消息响应
///@param session_id 资金账户对应的session_id，登录时得到
///@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
virtual void OnQueryOfferTgwId(EMTQueryOfferTgwIdRsp *offer_tgw_info, EMTRI *error_info, int request_id, bool is_last, uint64_t session_id) {};
```

### 4.1.67. <a id="onquerydmatickerassigninfobypage">OnQueryDMATickerAssignInfoByPage</a>

```cpp
///查询DMA业务融入股份信息的响应，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
///@param assign_info DMA业务融入股份信息
///@param req_count 请求到的最大数量
///@param rsp_sequence 分页请求的当前回报数量
///@param query_reference 当前信息所对应的查询索引，需要记录下来，在进行下一次分页查询的时候需要用到
///@param request_id 此消息响应函数对应的请求ID
///@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个
///@param session_id 资金账户对应的session_id，登录时得到
///@remark 当rsp_sequence为0，表明当次查询没有查到任何记录，当is_last为true时，如果rsp_sequence等于req_count，那么表示还有回报，可以进行下一次分页查询，如果不等，表示所有回报已经查询完毕一个查询请求可能对应多个响应，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线。
...
### 5.1.37. <a id="emt_repay_special_dept_type">还负债类型-EMT_REPAY_SPECIAL_DEPT_TYPE </a>

| 枚举                     | 取值 | 定义说明     |
|:------------------------|:------|:---------|
| EMT_REPAY_SPECIAL_DEPT  | '0'   | 一般负债     |
| EMT_REPAY_SPECIAL_FEE   | '1'   | 账户级费用    |